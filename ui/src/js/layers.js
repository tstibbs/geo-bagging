...
var bingDefaults = leaflet.extend(
	{maxZoom: 18, minZoom: 0, detectRetina: true},
	defaults
)
...
var bingFallbackLayer = new LeafletBing(
	leaflet.extend(
		{imagerySet: 'RoadOnDemand', maxZoom: 11, minZoom: 0},
		defaults
	)
) //note even though this layer supports retetina, having a group with a mix of retina and non-retina screws up the zooming
var bingOsGroup = leaflet.layerGroup([bingOsLayer, bingFallbackLayer])
//Bing standard maps
var bingRoads = new LeafletBing(
	leaflet.extend({imagerySet: 'RoadOnDemand'}, bingDefaults)
)
var bingHybrid = new LeafletBing(
	leaflet.extend({imagerySet: 'AerialWithLabelsOnDemand'}, bingDefaults)
)
var bingAerial = new LeafletBing(
	leaflet.extend({imagerySet: 'Aerial'}, bingDefaults)
)
//OSM
